...
>
> **属性。** 导航栏新增属性区。你可以选择要显示的仓库属性，再按这些 frontmatter 值筛选卡片流。每个值都会标出当前来源里有多少篇笔记使用它，没有该属性的笔记归入「未分配」。这些筛选在浏览文件夹时生效；进入卡片盒或双链视图时会暂停，回到文件夹后自动恢复。卡片盒规则也可以带上属性条件。
>
> **双链。** 双链区把当前笔记的出链和反链变成卡片来源。点开其中一项即可把这些笔记当成卡片浏览，可以把视图固定在当前笔记上，也可以继续跟随正在打开的笔记，还能把当前这组笔记存成卡片盒。
>
> **卡片分组。** 工具栏的排序与分组可以按编辑时间、创建时间或文件名排序，并按文件夹、标签、卡片盒规则或任务状态分组。分组可以按名称或卡片数量排列，也可以一起折叠或展开。带任务的卡片还会以「已完成/总数」显示进度。
