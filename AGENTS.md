...
- **Projection rule**: `src/view/pipeline.ts` is the only visible-card projection path. A scope load keeps the last committed scope/card stream and navigation mounted while it builds the complete lightweight replacement; it projects once and publishes the new scope, cards, and navigation together before startup preview work finishes. Startup paths are the first 6 visible cards, and the 120ms timer still marks late preview work. Folders: `tag filter -> property filter -> search filter -> pin reorder`. Boxes and links skip the browse tag and property filters and run `search -> pin` (box rule property clauses are digested at the membership layer). Browse tag/property filtering is a folder-only capability: filters set before entering a box/links scope stay dormant (with a "filters paused" toolbar hint) and resume when returning to a folder
...
- `pipeline.ts` remains the only visible-card projection path. A load commits the complete lightweight card list and scope tag/property facets together; card publication does not wait for startup preview work. Property filters compose there for folder scopes only; in box scopes, rule property clauses are digested at the membership layer and the projection runs `search -> pin`.
...
- Startup preview prewarm is limited to the first 6 visible candidates. The 120ms timer still marks late startup work; the complete card publish does not wait for it.
- Recently visited folder scopes keep a bounded per-view cache of sorted `TFile` candidates. A warm switch skips folder traversal, sorting, and the cold timer turn; forced refresh and vault mutations bypass/invalidate it. Card records, filters, and grouping are still rebuilt for the requested scope.
...
- Plugin surfaces register synchronously; restored card scope foreground work is the complete card publish, and it still completes before `searchCoordinator.initialize()`. `reconcileInitial` schedules the vault tag scan with `scheduleIdleTask` at 10_000ms. Full-vault search scans then wait for an idle window.
