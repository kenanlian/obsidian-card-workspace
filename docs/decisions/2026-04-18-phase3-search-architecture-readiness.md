# Decision: 完成 phase3 搜索架构 readiness 接缝
...
- `main.ts` 已拥有 plugin-owned `NoIndexSearchService` 生命周期与初始化失败回退。
- `applySearchFilter()` 已支持 fallback search，并保持 search-before-pin invariant。
...
4. `SearchService` 由 `main.ts` plugin-owned 持有，当前实现是 fallback-first、no-index seam。
...
当前 `SearchService` 可以返回 `orderedPaths: null`，视图就继续使用本地 fallback filtering。这个设计让系统在没有索引时仍能工作，也避免初始化失败直接让搜索断掉。
...
- 搜索失败或服务不可用时，系统仍能退回 fallback 路径。
...
- `src/search/types.ts`
- `src/search/NoIndexSearchService.ts`
