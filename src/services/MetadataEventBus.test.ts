...
import { MetadataEventBus } from "./MetadataEventBus";
...
describe("MetadataEventBus", () => {
  ...
  it("V50 runs listeners in registration order", async () => {
    const bus = new MetadataEventBus();
    const log: string[] = [];
  ...
  it("V50 isolates a sync throw so later listeners still run and publish does not reject", async () => {
    ...
    expect(warn).toHaveBeenCalledWith("[Card Workspace] Metadata event listener failed.", boom);
  });
